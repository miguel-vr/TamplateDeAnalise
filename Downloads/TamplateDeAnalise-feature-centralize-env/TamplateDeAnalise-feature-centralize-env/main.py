...
from core.knowledge_base import KnowledgeBase, KnowledgeBaseRefresher
...
def create_components(
    settings: Settings,
) -> Tuple[IntakeWatcher, FeedbackWatcher, Optional[KnowledgeBaseRefresher]]:
    ...
    if settings.knowledge_refresh_on_startup:
        try:
            knowledge_base.refresh_category_documents()
            logging.info("Knowledge base refreshed on startup.")
        except Exception as exc:  # pragma: no cover - defensive logging
            logging.error("Failed to refresh knowledge base on startup: %s", exc)
    refresh_thread: Optional[KnowledgeBaseRefresher] = None
    if settings.knowledge_refresh_interval > 0:
        refresh_thread = knowledge_base.start_periodic_refresh(int(settings.knowledge_refresh_interval))
        if refresh_thread:
            logging.info(
                "Knowledge base periodic refresh enabled (interval=%ss).",
                settings.knowledge_refresh_interval,
            )
    ...
    return intake_watcher, feedback_watcher, refresh_thread
...
def main() -> None:
    ...
    try:
        intake_watcher, feedback_watcher, knowledge_refresher = create_components(settings)
    ...
    def shutdown_handler(*_args):
        ...
        if knowledge_refresher:
            knowledge_refresher.stop()
        sys.exit(0)
