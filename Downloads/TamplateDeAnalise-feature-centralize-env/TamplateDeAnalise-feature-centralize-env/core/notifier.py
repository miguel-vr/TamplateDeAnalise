            ...
            "actions": [
                {
                    ...
                    "url": self._safe_file_uri(zip_path),
                }
            ],
        }
        return card

    def _safe_file_uri(self, value: str) -> str:
        """Convert a filesystem path to a file URI that works across platforms."""
        if not value:
            return ""
        lowered = value.lower()
        if lowered.startswith("file://") or lowered.startswith("http"):
            return value
        try:
