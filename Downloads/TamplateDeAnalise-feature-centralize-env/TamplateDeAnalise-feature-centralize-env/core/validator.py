...
class Validator:
    ...
    def ensure_confidence(self, result: Dict, text: str, metadata: Dict) -> Dict:
        ...
        if current.get("confidence", 0) < self.threshold:
            logging.warning(
                "Confidence remains below threshold after %s attempts. Flagging as Nao identificada.",
                self.max_retries,
            )
            current["categoria"] = "Nao identificada"
            current.setdefault("nova_categoria_sugerida", "Categoria a ser definida pelo usuario")
            current.setdefault(
                "justificativa",
                current.get("justificativa", "")
                + "\nConfianca insuficiente. Recomenda-se revisao humana e eventual criacao de nova categoria.",
            )
