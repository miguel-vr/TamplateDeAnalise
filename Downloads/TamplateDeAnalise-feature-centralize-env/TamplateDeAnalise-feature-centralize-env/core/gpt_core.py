...
class GPTCore:
    ...
    def analyze_document(self, text: str, metadata: Dict) -> Dict:
        ...
        if self.offline_mode:
            ...
            i3 = {
                "explanation": primary.get("justificativa", "Analise heuristica baseada em similaridade."),
        ...
        messages = [
            {
                "role": "system",
                "content": (
                    "You are an expert corporate document classifier. "
                    "You received a previous classification with low confidence. "
                    "Re-evaluate it carefully using the observations below."
                ),
            },
            {
                "role": "user",
        ...
        messages = [
            {
                ...
                "content": (
                    "You are an expert corporate document classifier. "
                    "Classify each document by category and theme using the full context provided. "
        ...
        messages = [
            {
                "role": "system",
                "content": (
                    "You act as an independent auditor who validates document classifications. "
                    "Evaluate coherence, confidence and potential mistakes. Respond with a valid JSON object."
                ),
            },
            {"role": "user", "content": prompt},
        ]
        ...
        messages = [
            {
                "role": "system",
                "content": (
                    "You generate structured explanations (I3 layer - Insight, Impact, Inference) "
                    "for document classifications. Focus on clarity and be concise. Respond with a valid JSON object."
                ),
            },
            {"role": "user", "content": prompt},
        ]
    ...
    def _render_primary_prompt(
        ...
        template = {
            ...
            "instructions": {
                "objective": "Classify the document by primary category, theme and optional secondary categories.",
                "confidence_format": "Percentage value from 0 to 100.",
                "new_category_rule": "If no known category fits, propose a new category and justify the decision.",
                "context": context_summary,
                "known_categories": known_categories or ["tecnologia", "juridico", "financeiro", "compliance", "outros"],
                "knowledge_usage": (
                    "When you suggest a new category, describe clearly why it differs from the known categories. "
                    "Always justify decisions using explicit evidence from the document."
                ),
                "document_knowledge_guidance": (
                    "Consider the characteristic terms learned from confirmed documents in each category. "
                    "If the current document diverges strongly from that history, explain the difference."
                ),
                "feedback_guidance": (
                    "Review the human feedback history by category (approvals, rejections, reprocess requests). "
                    "Avoid repeating negative patterns (keywords_flagged) and highlight alignment with positive signals."
                ),
                "category_profiles": category_brief,
                "validation_layers": [
                    "Highlight evidence supporting the chosen category and cite relevant keywords.",
                    "List meaningful alternative categories with justification and degree of match.",
                    "State whether multiple categories are needed when there is strong overlap."
                ],
            },
    ...
    def _render_cross_prompt(
        ...
        template = {
            ...
            "instructions": {
                "task": "Audit the primary analysis highlighting agreement, confidence adjustments, and risks.",
                "expected_fields": {
                    "agreement": "string",
                ...
                "feedback_profiles": feedback_brief,
                "feedback_context": (
                    "Use the human feedback history to confirm confidence adjustments and risks before issuing the final judgement."
                ),
                "consistency_checks": [
                    "If you disagree with the proposed category, suggest an alternative and cite the evidence.",
                    "Assess whether multiple categories are required and provide a match score."
                ],
            },
    ...
    def _render_i3_prompt(
        ...
        template = {
            ...
            "feedback_profiles": feedback_brief,
            "instructions": {
                "objective": "Produce the I3 explanation (Insight, Impact, Inference) and justify the final score.",
                "expected_fields": {
                    "insight": "string",
                    "impact": "string",
                    "inference": "string",
                ...
                "feedback_traceability": (
                    "Relate the I3 explanation to the human feedback history (positive reasons and recurring alerts)."
                ),
                "traceability": [
                    "Provide a why-trace linking evidence to the rules and predefined categories.",
                    "List the key words that determined the decision and relate them to known history."
                ],
            },
        }
        return json.dumps(template, ensure_ascii=False)
    ...
    def _render_reinforcement_prompt(
        ...
        template = {
            ...
            "feedback_profiles": feedback_brief,
            "instructions": {
                "focus": "Search for additional evidence in the text to improve confidence. If that is not possible, propose a new category.",
                "fallback": "If uncertainty remains, return the category as 'Nao identificada' and suggest a plausible alternative.",
                "known_categories": known_categories,
                "category_profiles": category_brief,
                "document_knowledge_profiles": document_brief,
                "feedback_context": (
                    "Use the feedback history to review critical issues, flagged words, and previous reanalysis requests."
                ),
            },
    ...
    def _parse_response(self, response, fallback: Optional[Dict] = None) -> Dict:
        ...
        try:
            ...
            if not sanitized or sanitized[0] not in ("{", "["):
    ...
    def _combine_outputs(self, primary: Dict, cross: Dict, i3: Dict) -> Dict:
        ...
        category = primary.get("categoria_principal") or "Nao identificada"
        result = {
            "categoria": category,
            "tema": primary.get("tema", "Tema nao identificado"),
    ...
    def _format_similarity_context(self, similar: List[Tuple[Dict, float]]) -> str:
        if not similar:
            return "Sem correspondencias fortes em conhecimento local."
        summary = []
    ...
    def _offline_analysis(self, text: str, metadata: Dict, context_summary: str) -> Dict:
        ...
        if similar:
            ...
            return {
                ...
                "tema": best_entry.get("theme", "Tema baseado em historico"),
                "areas_secundarias": best_entry.get("areas_secundarias", []),
                "confianca": confidence,
                "justificativa": (
                    f"Classificacao inferida a partir de similaridade historica (score {score:.2f}). Contexto: {context_summary}"
                ),
                "motivos_chave": ["Resultado heuristico por similaridade textual."],
                "nova_categoria_sugerida": None,
            }
        return {
            "categoria_principal": "outros",
            "tema": "Tema heuristico",
            "areas_secundarias": [],
            "confianca": 55,
            "justificativa": "Nenhuma correspondencia forte encontrada. Classificacao padrao aplicada.",
            "motivos_chave": ["Resultado heuristico por ausencia de contexto historico."],
            "nova_categoria_sugerida": None,
        }
